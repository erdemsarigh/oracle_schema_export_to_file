...
db = cx_Oracle.connect('schema adı', 'password', 'tnsname')
ca = db.cursor() 
sch = ca.execute(ca.prepare("select username from all_users where username in ('schemaadı', 'OTHER_SCHEMA') order by username")).fetchall() 
